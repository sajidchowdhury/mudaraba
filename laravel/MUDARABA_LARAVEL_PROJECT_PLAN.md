...
1. **Seed data is January 2026, not July 2026** — ✅ **CLOSED (2026-09-08)**. Created a new `July2026Seeder` + `july_2026_data.json` that loads the canonical "July, 2026 For Sajid" reference data from the Excel sheet (Z2=1,765,000, X2=1,635,000, Y2=130,000 — matches the ParityTest expectations exactly). The seeder loads BOTH months' sector profits (January + July 2026) so users can navigate between months at `/profit/investor?month=YYYY-MM-01`. The 150 investors are reused from `january_2026_data.json` (real per-investor July 2026 balance data isn't in the codebase — see `july_2026_data.json` → `_meta.note_about_investors`). `DatabaseSeeder` now calls `July2026Seeder` by default; `January2026Seeder` is still available via `php artisan db:seed --class=January2026Seeder`. Added 9 new Pest tests in `ParitySeederTest.php` that run the parity check against the SEEDED data: verifies sector counts, investor counts, Z2/X2/Y2 match the canonical Excel sheet, the calculation engine produces the same totals, AG184 = X2 − AG182 identity holds, retained earnings (200K + 71/29 split) are correct, and the "For Sajid" grid has all ~150 investors. The original `ParityTest.php` is unchanged (it creates its own hand-coded data and still verifies the algorithm). `refresh.sh` updated to show both months' totals.
...
9. **Soft deletes** — ✅ **CLOSED (2026-09-08)**. Audited all 30 Eloquent models and 35 migrations. Found: 12 models correctly use `SoftDeletes` trait with matching `softDeletes()` columns in their migrations (InvestmentTransaction, SectorInvestment, DirectorTransaction, MonthlySectorProfit, Investor, Sector, Director, User, Employee, Menu, AdvanceProfitAdjustment, ProfitAdjustment). 18 models without `SoftDeletes` are correctly excluded (10 ledger tables = running balances, AuditLog = append-only, InvestorMonthlyProfitDetail + MonthlyProfitSummary = intentional append-only snapshots, UserPermission = junction table, RetainedEarnings/Distribution = derived config, 2 legacy TypeA/TypeB = dropped tables). **Gap found**: `updated_by` and `deleted_by` columns were missing from all tables (plan §4.1 calls for "Universal `created_by`, `updated_by`, `deleted_by`"). Fixed with: (1) new migration `2026_09_08_220000_add_updated_by_deleted_by_to_financial_tables.php` adding nullable FK `updated_by` + `deleted_by` columns to 9 financial tables; (2) updated `Auditable` trait to auto-set `updated_by = Auth::id()` in the `updating` event and `deleted_by = Auth::id()` in the `deleting` event (before soft-delete fires); (3) added `hasColumn()` helper to safely check for these columns before setting them (avoids "column not found" errors on tables without the columns).
10. **Performance / virtualization (Phase 8.3)** — ✅ **CLOSED (2026-09-08)**. Added `@tanstack/react-virtual` for row virtualization of the 150+ investor profit grid. The `InvestorProfit/Index.tsx` component now uses `useVirtualizer` from TanStack Virtual to render only visible rows + 8 rows of overscan when the grid has more than 50 investors. For smaller grids (≤ 50 rows), the plain `grid.map()` path is used (simpler, no virtualizer overhead). Implementation details: (1) added `@tanstack/react-virtual` v3.10 to `package.json` dependencies; (2) the scroll container is the table's wrapper div with `max-h-[600px]` + `overflow-auto`; (3) `table-layout: fixed` ensures consistent column widths during scroll; (4) sticky header (`position: sticky; top: 0`) stays visible during vertical scroll; (5) padding `<tr>` elements before/after the visible rows maintain the scrollbar height to match the full 150+ row dataset; (6) `React.Fragment` replaces the previous `<>` shorthand for row key assignment (required for virtualized rows); (7) `estimateSize: 49` (approximate row height matching `h-12` + padding) with `overscan: 8` for smooth scrolling.
...
- ✅ **Audit log**: writes a single `reconcile` audit row per month per user action (regardless of investor count) via `AuditService::log(ACTION_RECONCILE, ...)` — see Gap #8 in the Progress Tracker
