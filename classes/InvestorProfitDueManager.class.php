...
class InvestorProfitDueManager extends Dbh
{
    ...
    public function updateDue($investor_id, $amount, $due_month)
    {
        ...
        try {
            ...
            // Daily due
            $stmt = $pdo->prepare("INSERT INTO investor_profit_monthly_due ( investor_id, due_month, due)
            ...
            // Ledger due
            $stmt = $pdo->prepare("INSERT INTO investor_profit_due_ledger ( investor_id, due)
    ...
    {
        $stmt = $this->connect()->prepare("SELECT due FROM investor_profit_due_ledger WHERE  investor_id = ?");
    ...
    {
        $stmt = $this->connect()->prepare("REPLACE INTO investor_profit_due_ledger ( investor_id, due) VALUES (?, ?, ?)");
        $stmt->execute([ $investor_id, $amount]);
    }

   

public function getPreviousDueByDate($investor_id, $date_from)
{
    // Convert input date to YYYY-MM
    $targetMonth = (new DateTime($date_from))->format('Y-m');

    $sql = "SELECT due 
            FROM investor_profit_monthly_due
            WHERE investor_id = ?
              AND due_month < ?
    ...
    $stmt->execute([$investor_id, $targetMonth]);
...
{
    $stmt = $this->connect()->prepare("SELECT due FROM investor_profit_due_ledger 
        WHERE  investor_id = ?");
    $stmt->execute([ $investor_id]);
    return (float)($stmt->fetchColumn() ?: 0);
}
...
{
    $stmt = $this->connect()->prepare("SELECT MAX(due) FROM investor_profit_monthly_due WHERE  investor_id = ?");
    $stmt->execute([ $investor_id]);
