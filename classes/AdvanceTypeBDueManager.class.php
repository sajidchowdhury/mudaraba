...
class AdvanceTypeBDueManager extends Dbh
{
    /**
    ...
    public function updateFund($amount, $date = null)
    {
        ...
        try {
            ...
            $stmt = $pdo->prepare("
                INSERT INTO adv_profit_adjusting_fund_type_B (`date`, `amount`)
                VALUES (:date, :amount)
    ...
    public function getFundByDate($date = null)
    {
        ...
        $stmt = $pdo->prepare("SELECT amount FROM adv_profit_adjusting_fund_type_B WHERE `date` = ?");
        $stmt->execute([$date]);
    ...
    {
        $pdo = $this->connect();
        $stmt = $pdo->query("SELECT amount FROM adv_profit_adjusting_fund_type_B ORDER BY `date` DESC LIMIT 1");
        return (float)($stmt->fetchColumn() ?: 0.00);
    }
    ...
    {
        $pdo = $this->connect();
        $stmt = $pdo->query("SELECT SUM(amount) FROM adv_profit_adjusting_fund_type_B");
        return (float)($stmt->fetchColumn() ?: 0.00);
    }
}
