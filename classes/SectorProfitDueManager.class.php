...
class SectorProfitDueManager extends Dbh
{
    ...
    public function updateDue($sector_id, $amount, $due_month)
    {
        ...
        try {
            ...
            // Daily due
            $stmt = $pdo->prepare("INSERT INTO sector_profit_monthly_due ( sector_id, due_month, due)
            ...
            // Ledger due
            $stmt = $pdo->prepare("INSERT INTO sector_profit_due_ledger ( sector_id, due)
    ...
    {
        $stmt = $this->connect()->prepare("SELECT due FROM sector_profit_due_ledger WHERE  sector_id = ?");
    ...
    {
        $stmt = $this->connect()->prepare("REPLACE INTO sector_profit_due_ledger ( sector_id, due) VALUES (?, ?, ?)");
...
{
    $stmt = $this->connect()->prepare("SELECT due FROM sector_profit_due_ledger 
        WHERE  sector_id = ?");
    $stmt->execute([ $sector_id]);
    return (float)($stmt->fetchColumn() ?: 0);
}
...
{
    $stmt = $this->connect()->prepare("SELECT MAX(due) FROM sector_profit_monthly_due WHERE  sector_id = ?");
    $stmt->execute([ $sector_id]);
